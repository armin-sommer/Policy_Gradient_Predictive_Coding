...
### PCPG runs and the settling ablation

`--algos pc_actor_critic` / `pc_reinforce` run the PC algorithms instead of the
baselines. `--tier` picks the frozen config (`bench` / `sota`) and `--arms` picks the
settling arm:

```bash
# the settling ablation — both arms, paired by seed, on GPU
modal run modal_app.py --algos pc_actor_critic --arms both --seeds 1,2,3 \
    --envs halfcheetah --tier bench

# settled only (what the committed configs now do)
modal run modal_app.py --algos pc_reinforce --arms settled --seeds 1,2,3
```

`--arms both` prints a paired settled-vs-unsettled contrast at the end. `--arms` is
ignored for `ppo`/`trpo`, which have no settling arm.

**Why this matters:** every committed PCPG result was produced ~1% settled (see
`docs/PCPG_GEOMETRY_FINDINGS.md` §4d). On the bandit, settling measurably *slows*
learning (§4e); whether MuJoCo behaves the same way is the open question this run
answers.

PCPG logs land in `/results/pcpg_modal/<env>_<algo>_<tier>_<arm>_seed<n>.log` — the
arm is in the filename because it is the variable under test and must not be
ambiguous when results are attributed later. Baseline logs keep `run_bench.sh`'s
naming so they interleave with the committed seeds 1–3.

The image installs `jpc`, `equinox` and `diffrax` (pinned in
`requirements-gpu.txt`); without them only the backprop baselines can run.
