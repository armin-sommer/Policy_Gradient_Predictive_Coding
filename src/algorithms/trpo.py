...
def make_inference_fn(agent_networks: Networks):
    ...
    def make_policy(params: Any, deterministic: bool = False) -> Policy:
        ...
        @jax.jit
        def policy(observations: jnp.ndarray, key_sample: jnp.ndarray):
...
def compute_gae(truncation, termination, rewards, values, bootstrap_value,
                lambda_=1.0, discount=0.99):
