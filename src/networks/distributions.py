...
class NormalTanhDistribution(ParametricDistribution):
    ...
    def __init__(self, event_size, min_std=0.001, exp_std=False):
        """Initialize the distribution.

        Args:
        event_size: the size of events (i.e. actions).
        min_std: minimum std for the gaussian.
        exp_std: if True use scale = exp(log_std) (SOTA: log_std=0 -> std=1);
            otherwise softplus(scale) + min_std (default).
        """
...
class DiscreteDistribution(abc.ABC):
    ...
    def sample_no_postprocessing(self, parameters, seed):
        return jax.random.categorical(seed, parameters)
    ...
    def log_prob(self, parameters, actions):
        """Compute the log probability of actions. Supports any leading batch dims."""
        log_probs = jax.nn.log_softmax(parameters)
    ...
    def entropy(self, parameters, seed):
        """Return the entropy of the given distribution."""
        log_probs = jax.nn.log_softmax(parameters)
        return -jnp.sum(jax.nn.softmax(parameters) * log_probs, axis=-1)

    def kl_divergence(self, p_parameters, q_parameters):
        """Return the KL divergence of the given distributions."""
        p_probs = jax.nn.softmax(p_parameters)
        p_log = jax.nn.log_softmax(p_parameters)
        q_log = jax.nn.log_softmax(q_parameters)
        return jnp.sum(p_probs * (p_log - q_log), axis=-1)