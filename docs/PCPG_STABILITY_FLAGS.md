...
| `inference_rate_correction` | bool (`False` in code, **`true` in every `configs/mujoco_*_pc_*.yaml`**) | `train.inference_rate_correction` | `--inference-rate-correction` / `--unsettled` | `_settled` / `_unsettled` | Run the inference ODE at per-sample rate. jpc's energy is a batch mean, so `dz/dt = −∂F/∂z` carries a `1/N` that divides every activity's velocity: `max_t1` is an ODE *end time*, and the per-sample time is `t₁/N` — `20/2048 ≈ 0.01` at bench scale, i.e. **~1% settled**. Multiplying the vector field by `N` cancels it, so `max_t1` *is* the per-sample time and 20 reaches 99.5%. The fixed point is unchanged (dividing a gradient flow by a constant cannot move its zero — verified `cos = 1.000000` vs jpc integrated to `t₁ = 40N`), and the weight step keeps the `1/N` so learning rates carry over. Raising `max_t1` instead cannot work: jpc hard-stops at `t = 4096` (93.8% at best) and convergence needs ~20,480. |
